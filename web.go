...
`
	...
	if err != nil {
		var backupReadMe string
		backupReadMe, err = readFile("ReadMe.html")
		if err != nil {
			fail("No ReadMe.conf or ReadMe.html file found!")
		} else {
			var useHTMLReadMe bool = ask("No ReadMe.conf detected, but a ReadMe.html was found. Do you want to use the raw contents of ReadMe.html as the ReadMe instead of the autogenerated ReadMe?")
			if useHTMLReadMe {
				info("Writing to ReadMe.html...")
				writeFile(dirPath+"assets/ReadMe.html", backupReadMe)
			} else {
